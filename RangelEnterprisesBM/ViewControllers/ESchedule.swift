        ...
        for i in 0..<self.user!.numDaysScheduled {
            ...
            scheduledRef.queryOrderedByKey().queryEqual(toValue: i.description).observeSingleEvent(of: .value) { (snapshot) in
                if snapshot.exists() {
                    ...
                    let tempYear = self.cal.component(.year, from: tempScheduledDay.date)
                    let tempMonth = self.cal.component(.month, from: tempScheduledDay.date)
                    let tempDay = self.cal.component(.day, from: tempScheduledDay.date)
                    
                    if tempDay == selectedDay && tempMonth == selectedMonth && tempYear == selectedYear {
